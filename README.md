...
![gpx-downloader.html, connected to a T1000-E, showing live battery/temp/light plus the scan and LoRa drain interval config section](docs/gpx-downloader-screenshot.png)

**Scan & LoRa Drain interval, over BLE (v32).** The page also has a Config section: **Read Config** fetches the current scan and drain intervals straight from the tracker (`AT+POS_INT=?` / `AT+DRAIN_INT=?`) and shows them as plain text, **Save Config** prompts for a new value for each (pre-filled with the last-read value) and writes them back (`AT+POS_INT=<min>` / `AT+DRAIN_INT=<min>`) — they take effect immediately, no reboot, no LoRaWAN downlink round trip needed. Same commands work from any BLE terminal app too. `AT+DRAIN_INT` is new in v32; `AT+POS_INT` already existed but used to only persist to flash for the next boot — fixed to apply live at the same time, matching how the v31 downlink already behaved for the same field.

The values display as read-only text with a `window.prompt()` dialog for entering new ones, rather than editable text fields — deliberately, since the CMS hosting the live copy strips `<input>` tags entirely out of pasted HTML (a security measure against embedding form fields in custom content blocks). Confirmed by inspecting the live page's DOM directly: `document.querySelectorAll('input')` came back empty even though the surrounding buttons rendered fine. Same lesson as the backslash-stripping fix earlier in v31/v32 — anything in this page has to survive whatever that CMS's sanitizer does to it, not just work in a plain browser.
